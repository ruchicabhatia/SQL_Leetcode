...
SELECT 
X.TRIP_DATE AS 'Day',
ROUND((X.CANCELLED_REQUESTS/X.ID),2) AS 'Cancellation Rate'
FROM
( 
	SELECT
	T.REQUEST_AT AS TRIP_DATE,
	SUM(CASE
	...
	COUNT(T.ID) AS ID
	FROM Trips T
	WHERE T.CLIENT_ID IN (SELECT USERS_ID FROM USERS U WHERE U.BANNED = 'NO')
		AND T.DRIVER_ID = (SELECT USERS_ID FROM USERS U1 WHERE U1.BANNED = 'NO')
